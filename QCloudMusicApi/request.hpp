...
static QString chooseUserAgent(QString ua = "") {
    ...
    QList<QString> list = userAgentList["mobile"].toStringList();
	list.append(userAgentList["pc"].toStringList());
	auto i = userAgentList["mobile"].toStringList().size();
	QList<QString> realUserAgentList = userAgentList[ua].isValid() ? userAgentList[ua].toStringList() : list;
    return QList<QString>({"mobile", "pc", ""}).indexOf(ua) > -1 ? realUserAgentList[QRandomGenerator::global()->generate() % realUserAgentList.size()]
                                                        : ua;
}
